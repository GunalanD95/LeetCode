...
class Solution:
    ...
    def addRightBoundary(self,root):
        cur_node = root.right
        temp = []
        while cur_node:
            if not self.isLeaf(cur_node):
    ...
    def printBoundaryView(self, root):
        # Code her
        if not root: return []
        if not self.isLeaf(root): self.res.append(root.data)
        
        self.addLeftBoundary(root)
        self.addLeaves(root)
        self.addRightBoundary(root)
